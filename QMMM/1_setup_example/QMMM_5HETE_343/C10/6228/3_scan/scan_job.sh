...
cp $SLURM_SUBMIT_DIR/*6228* $TMP_DIR             # prmtop, inpcrd, c, pdb, act_list
...
# scan
echo "6228    starting QM/MM scan @ `date`"
cd $TMP_DIR/
srun chemsh scan.chm > $SLURM_SUBMIT_DIR/scan.out
echo "6228    finished QM/MM scan @ `date`"
