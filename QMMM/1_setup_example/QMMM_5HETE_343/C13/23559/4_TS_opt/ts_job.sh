...
cp $SLURM_SUBMIT_DIR/*23559* $TMP_DIR              # prmtop, inpcrd, pdb, act_list
...
# optimization
echo "23559    starting QM/MM TS optimization @ `date`"
cd $TMP_DIR/
srun chemsh ts.chm > $SLURM_SUBMIT_DIR/ts.out
echo "23559    finished QM/MM TS optimization @ `date`"
...
echo "23559    finished TS optimization job @ `date`"
echo ""